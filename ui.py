...
class View(QWidget):
    ...
    def initUI(self):
        ...
        hbox_formular.addWidget(self.le1)  # QLineEdit 위젯 추가
        hbox_formular.addWidget(self.cb)  # QComboBox 위젯 추가
        hbox_formular.addWidget(self.le2)  # QLineEdit 위젯 추가

        hbox = QHBoxLayout()  # 수평 박스 레이아웃 생성
        hbox.addStretch(1)  # 공간 추가
        hbox.addWidget(self.btn1)  # QPushButton 위젯 추가
        hbox.addWidget(self.btn2)  # QPushButton 위젯 추가

        vbox = QVBoxLayout()  # 수직 박스 레이아웃 생성
        vbox.addWidget(self.te1)  # QPlainTextEdit 위젯 추가
        vbox.addLayout(hbox_formular)  # 수평 박스 레이아웃 추가
        vbox.addLayout(hbox)  # 수평 박스 레이아웃 추가
    ...
    def setDisplay(self, text):
        self.te1.appendPlainText(text)  # QPlainTextEdit에 텍스트 추가
